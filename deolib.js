...
dl.Button = class {
  ...
  constructor(bounds, options = {}) {
    ...
    this.borderThick = options.borderThick || Math.max(1, Math.floor((bounds.width + bounds.height) / 80));
  ...
  updateText(newText = this.text) {
    ...
    _ctx.save();
      ...
      this.cachedLineHeights = this.textLines.map(line => {
        ...
        const descent = metrics.actualBoundingBoxDescent ?? this.fontSize * 0.5;
  ...
  isPressed() {
    ...
    if (!this.isDisabled) {
      ...
      if (MBReleased) {
        if (this.pressedInside && mouseOver) clicked = true;
        this.pressedInside = false;
      }
    ...
    this.state = "normal";
    if (this.isDisabled) this.state = "disabled";
    else if (this.pressedInside) this.state = "pressed";
    else if (mouseOver) this.state = "focused";
  ...
  draw() {
    const [innerColor, outerColor, textColor] = this.style[this.state];

    if (this.roundness) {
      dl.drawRectangleRoundedRec(this.bounds, this.roundness, this._borderColor||outerColor);
      dl.drawRectangleRoundedRec(this.innerRect, this.roundness, innerColor);
    } else {
      dl.drawRectangleRec(this.bounds, this._borderColor||outerColor);
